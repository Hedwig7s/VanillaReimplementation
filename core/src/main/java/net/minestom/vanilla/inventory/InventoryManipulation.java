...
public class InventoryManipulation {
    ...
    public static void damageItemIfNotCreative(Player player, Player.Hand hand, int amount) {
        ...
        if (!itemStack.has(ItemComponent.DAMAGE) || !itemStack.has(ItemComponent.MAX_DAMAGE)) return;

        int damage = itemStack.get(ItemComponent.DAMAGE);
        int maxDamage = itemStack.get(ItemComponent.MAX_DAMAGE);
