...
interface NBTPathImpl extends NBTPath {
    ...
    /**
     * Selects an arbitrary number of elements from a provided BinaryTag element.
     */
    interface Selector<T extends BinaryTag> {

        /**
         * Provides each selected BinaryTag element from {@code source} into {@code selectedElements}.
         *
         * @param source           the reference that is the source BinaryTag elemen  t
         * @param selectedElements the consumer for selected BinaryTag references
         */
    ...
    /**
     * Selects a single element (or none) from a provided BinaryTag element.
     *
     * @param <T>
     */
    interface SingleSelector<T extends BinaryTag> extends Selector<T> {
        ...
        /**
         * Provides the selected BinaryTag element from {@code source}.
         *
         * @param source the reference that is the source BinaryTag element
         * @return the selected BinaryTag element
         */
        ...
        while (true) {
            ...
            if (!VALID_SELECTOR_STARTERS.contains(peek(reader))) {
                if (selectors.isEmpty()) {
                    reader.reset();
                    String message = "BinaryTag paths must contain at least one selector (reading from " + reader + ")";
            ...
            if (selector == null) {
                reader.reset();
                String message = "Invalid BinaryTag path selector (reading from " + reader + ")";
                throw new IllegalArgumentException(message);
            }
